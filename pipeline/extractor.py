...
class ContentExtractionStage(PipelineStage):
    ...
    def process_item(self, page: Page, job: CrawlJob) -> Page:
        """Extract content using best available method (Crawl4AI or BeautifulSoup)"""
        try:
            # Use Crawl4AI content if available (Phase 3B enhancement)
            if page.browser_fetched and page.crawl4ai_markdown:
                self.logger.debug(f"Using Crawl4AI content for {page.url}")
            ...
            self.logger.debug(f"Extracted content from {page.url}: "
                            ...
                            f"quality: {page.markdown_quality_score:.2f}")
    ...
    def _extract_from_crawl4ai(self, page: Page) -> Page:
        """Extract content from Crawl4AI results (Phase 3B)"""
        # Markdown is already generated by Crawl4AI
        page.markdown_content = page.crawl4ai_markdown

        # Extract title from cleaned HTML or original HTML
        ...
        # Generate clean text from markdown (strip markdown syntax)
        page.clean_content = self._markdown_to_text(page.markdown_content)

        # Ensure extraction method is set
    ...
    def _extract_with_beautifulsoup(self, page: Page) -> Page:
        """Extract content using BeautifulSoup (original implementation)"""
        soup = BeautifulSoup(page.html_content, 'html.parser')
    ...
    def _markdown_to_text(self, markdown: str) -> str:
        """Convert markdown to plain text for analysis (Phase 3B)"""
        if not markdown:
            return ""
    ...
    def _calculate_quality_score(self, page: Page) -> float:
        """Calculate content quality score (0-1) (Phase 3B)"""
        score = 0.0
        ...
        # Has substantial content (30%)
        if page.clean_content:
            content_len = len(page.clean_content)
            if content_len > 1000:
                score += 0.3
        ...
        # Markdown has structure (25%)
        if page.markdown_content:
            # Has headers
            if '##' in page.markdown_content or '###' in page.markdown_content:
                score += 0.1
            # Has lists
            if '- ' in page.markdown_content or '* ' in page.markdown_content or re.search(r'^\d+\.', page.markdown_content, re.MULTILINE):
                score += 0.1
            # Has links
            if '[' in page.markdown_content and '](' in page.markdown_content:
                score += 0.05

        # Content/markdown ratio (25%)
        if page.clean_content and page.markdown_content:
            # Good markdown should be longer than clean text due to formatting
            ratio = len(page.markdown_content) / max(len(page.clean_content), 1)
            if 1.0 <= ratio <= 2.0:  # Reasonable ratio
                score += 0.25
            elif 0.8 <= ratio < 1.0 or 2.0 < ratio <= 3.0:  # Acceptable ratio
                score += 0.15
            else:  # Poor ratio
                score += 0.05
